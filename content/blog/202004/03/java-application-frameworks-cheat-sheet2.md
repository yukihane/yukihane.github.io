...
[前回](/posts/202004/02/java-application-frameworks-cheat-sheet/)はアプリケーションサーバにデバッガをアタッチする方法までを説明しました。
...
Javaにおいて、いわゆるオレオレフレームワークはServletコンテナを用いた [Fornt Controller Pattern](https://www.oracle.com/technetwork/java/frontcontroller-135648.html) で実現されることが一般的です。

そういったフレームワークとServletコンテナとの接続ポイントが、 `web.xml` で設定されます。ですので、 `web.xml` にかかれているクラス名を見れば利用されているフレームワークの種類がわかります。

- [Spring Framework(Spring MVC)](https://docs.spring.io/spring/docs/5.2.5.RELEASE/spring-framework-reference/web.html#mvc-servlet)

- [JSF(Mojarraの例)](https://eclipse-ee4j.github.io/mojarra/#controller)

- [CDI(Weldの例)](https://docs.jboss.org/weld/reference/latest/en-US/html/weldexamples.html#_the_numberguess_example_in_apache_tomcat_or_jetty)
