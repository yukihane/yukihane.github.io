...
- [Rustのインストール](https://www.rust-lang.org/ja-JP/install.html) およびここからたどれる <https://github.com/rust-lang-nursery/rustup.rs/blob/master/README.md#working-with-rust-on-windows>\[rustup

  - Working with Rust on Windows\]

- IDE(Eclipseベース): [RustDT Installation](https://github.com/RustDT/RustDT/blob/latest/documentation/Installation.md)およびここからたどれる [User Guide - Configuration](https://github.com/RustDT/RustDT/blob/latest/documentation/UserGuide.md#configuration)
...
[Rust and IDEs · The Rust Forge](https://forge.rust-lang.org/ides.html) にIDEの一覧がある。
...
- 次のコマンドを実行する([参考](https://github.com/RustDT/RustDT/blob/latest/documentation/UserGuide.md#configuration)):
...
[Oracle JDK](http://www.oracle.com/technetwork/java/javase/downloads/index.html)をインストールする。

[Eclipseアーカイブダウンロードページ](http://archive.eclipse.org/eclipse/downloads/) \> 最新バージョン \> Platform Runtime Binary セクションの該当OSバイナリ をダウンロードして展開する。
...
| 項目名               | 入力値                                                                                 |
|----------------------|----------------------------------------------------------------------------------------|
| Directory            | %USERPROFILE%\\rustup\toolchains\stable-x86_64-pc-windows-gnu                          |
| Rust 'src' Directory | %USERPROFILE%\\rustup\toolchains\stable-x86_64-pc-windows-gnu\lib\rustlib\src\rust\src |
...
| Rainicorn parse_describe | %USERPROFILE%\\cargo\RustDT\bin\parse_describe.exe |
...
- [Rust Win GNU ABI で実際何か作る時は MinGW-w64 の gcc にパスを通しておくのが実質必須と言うお話 - 日曜プログラミング](http://sunday-programming.hatenablog.com/entry/2016/05/07/231639)

これらはMSYS2を用いてセットアップします。[MSYS2インストールを参照してください](MSYS2インストール)。
...
対処法は \`tests/dummy.rs\`に空ファイルを生成すること。テストコードが1つもない場合上の事象になってしまうようだ([参考](https://github.com/RustDT/RustDT/issues/30#issuecomment-235007244)。なお他にもボタンを押したときの実行コマンドラインを変更するといったワークアラウンドもある模様 )。
