...
- [NVIDIAとIntelでマルチディスプレイ - 暇つぶし文@謎](http://d.hatena.ne.jp/nazodane/20170125/1485360393)

- [How to run both Intel and Nvidia graphics card driver on dual monitor setup? - Ask Ubuntu](https://askubuntu.com/questions/593938/how-to-run-both-intel-and-nvidia-graphics-card-driver-on-dual-monitor-setup)

- [Chapter 32. Offloading Graphics Display with RandR 1.4 - NVIDIA Accelerated Linux Graphics Driver README and Installation Guide](http://us.download.nvidia.com/XFree86/Linux-x86/319.12/README/randr14.html)
...
### [Proprietary GPU Drivers PPA](https://launchpad.net/~graphics-drivers/+archive/ubuntu/ppa) の登録
...
で\`nvidia\`を名前に含むパッケージ一覧が検索できるので、ついている数値の一番大きい(一番最新の)ものをインストールする。 今回は**410**だった。
...
で\`nvidia\`を名前に含むパッケージ一覧が検索できるので、\`nvidia-nnn\`の\`nnn\`が一番大きいもの(つまり最新のドライバ)をインストールする。 今回は**390**だった。
