...
設定画面からShift+Spaceに変更すると、逆方向の切り替えが同期してSpaceになってしまい使い物にならない。 次のコマンドで変更できる。
...
Eclipse上の表示がおかしいので別のfontを用いることにした。 今回インストールしたのは [Cikaフォント](https://github.com/miiton/Cica/releases)。

1.  .ttf ファイルを /usr/local/share/fonts ディレクトリにコピーする

2.  `sudo fc-cache -fv` コマンドを実行する
